...
def coordinationFGPv1(atoms, scale=1.25):
    '''
    Only applies to micro canonical ensemble
    ...
    myFGP = np.array([
        ...
        bl.min(),                       # 9:  min of bond length
        bl.max(),                       # 10: max of bond length
    ])
    return myFGP

def posMatEigenFGP(atoms):
    '''
    Only applies to micro canonical ensemble
    '''
    posMat = atoms.get_all_distances(mic=True)
    eigVal, eigVec = np.linalg.eigh(posMat)
    return eigVal
