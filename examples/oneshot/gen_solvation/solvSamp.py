...
if __name__ == "__main__":
    ...
    while len(traj) < num_configs_to_generate:
        ...
        cell = surf.get_allAtoms().cell
        simulation_box_solute = np.array([
            [0, cell.lengths()[0]],
            [0, cell.lengths()[1]],
            [9, 14]]) # you need to specify the Z range
        simulation_box_solvent = np.array([
            [0, cell.lengths()[0]],
            [0, cell.lengths()[1]],
            [9, 20]]) # you need to specify the Z range

        # Place the solute first, close to surface
        slab_solvated = gen_solv_layer(
            surf,
            [Atoms([cation], [[0,0,0]])],
            simulation_box_solute,
            toler=0.75,
            allowed_bonds=allowed_bonds
        )
        
        if slab_solvated is None: continue

        # Place the solvent molecules to solvate
        slab_solvated = gen_solv_layer(
            slab_solvated,
            [molecule('H2O')] * 40,
            simulation_box_solvent,
            toler=0.75,
            allowed_bonds=allowed_bonds
        )
